...
export class KhoComponent extends BaseComponent implements OnInit,AfterViewInit {
  ...
  public LoadData() {
    this._api.post('api/Kho/search', { page: 1, pageSize: 10,tennv: this.frmSearch.value['txt_tennv']}).subscribe(res => {
  ...
  public loadUnit(){
    this._api.get('/api/Kho/get').subscribe(res => {
      this.list_unit = res;
    });
  }

  public loadProducer(){
    this._api.get('/api/Kho/getAll').subscribe(res => {
 ...
 console.log(this.list_product)
  ...
  public openUpdateModal(id: any) {
    ...
    setTimeout(() => {
      $('#createModal').modal('toggle');
      this._api.get('/api/Kho/get-by-id/' + id).subscribe(res => {
        this.product = res;
  ...
  public onRemove(id: any) {
    this._api.delete('/api/Kho/delete-kho', id).subscribe(res => {
...
//         });
//       } else {
        this._api.post('/api/Kho/create-kho', kho).subscribe(res => {
          if (res && res.data) {
            alert('Thêm dữ liệu thành công');
            this.loadProduct();
            this.closeModal();
      ...
      //   });
      // } else {
        this._api.post('/api/Kho/update-kho', kho).subscribe(res => {
          if (res && res.data) {
            alert('Cập nhật dữ liệu thành công');
            this.loadProduct();
            this.closeModal();
  ...
  let files:any = document.getElementById('file');
   ...
   this._api.post('/api/Kho/create-kho',sp).subscribe(res=>{
         alert("ok");
   })
