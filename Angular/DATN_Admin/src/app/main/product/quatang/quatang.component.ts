...
export class QuatangComponent extends BaseComponent implements OnInit,AfterViewInit  {
  ...
  constructor(injector: Injector) {
    ...
    this.frmSearch = new FormGroup({
      'txt_tenqt': new FormControl(''),
      'txt_maqt': new FormControl(''),
    });
  }
  ...
  public loadUnit(){
    this._api.get('/api/Quatang/getAll').subscribe(res => {
      this.list_unit = res;
    });
  }
  ...
  public loadProduct(){
    this._api.get('/api/Quatang/getAll'
    ).subscribe(res => {
 ...
 console.log(this.list_product)
  ...
  public openUpdateModal(id: any) {
    ...
    setTimeout(() => {
      $('#createModal').modal('toggle');
      this._api.get('/api/Quatang/get-by-id/' + id).subscribe(res => {
        this.product = res;
        console.log(this.product)
        this.doneSetupForm = true;
        this.frmProduct = new FormGroup({

          'txt_tenqt': new FormControl(this.product.tenQt, ),
          'txt_maqt': new FormControl(this.product.maQt,),
          'txt_masp': new FormControl(this.product.maSp,),
          'txt_mota': new FormControl(this.product.mota,),
          'txt_trangthai': new FormControl(this.product.trangThai,),
        });
  ...
  public onRemove(id: any) {
    this._api.delete('/api/Quatang/delete-quatang', id).subscribe(res => {
  ...
  OnSubmit(vl: any) {
    debugger;
    let quatang = {
      MaQt: Number(vl.txt_maqt),
      MaSp: vl.txt_masp,
      TenQt: vl.txt_tenqt,
      Mota: vl.txt_mota,
      TrangThai: Number(vl.txt_trangthai),
    }
    console.log(quatang)
    if (this.isCreate) {
//       if (this.file) {
...
//         });
//       } else {
        this._api.post('/api/Quatang/create-quatang', quatang).subscribe(res => {
          if (res && res.data) {
            alert('Thêm dữ liệu thành công');
            this.loadProduct();
            this.closeModal();
      ...
      quatang.MaQt = this.product.maQt;
      // if (this.file) {
      //   this._api.uploadFileSingle('/api/upload/upload-single', 'product', this.file).subscribe((res: any) => {
      //     if (res && res.body && res.body.filePath) {
      ...
      //   });
      // } else {
        this._api.post('/api/Quatang/update-quatang', quatang).subscribe(res => {
          if (res && res.data) {
            alert('Cập nhật dữ liệu thành công');
            this.loadProduct();
            this.closeModal();
